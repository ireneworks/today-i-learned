...
리액트의 diff란 이전 버전의 가상 돔과 최신 버전의 가상 돔을 비교한다. DFS 알고리즘으로 비교하면서 Real DOM 업데이트가 필요한 노드가 있다면 flag를 달아두고 모은다. 렌더가 모두 끝나면 변경이 필요한 노드들을 모두 모은 Effect list를 얻을 수 있다.



**비교**

Effect(Hooks)는 랜더링 단계에서 컴포넌트를 마운트/언마운트하면서 발생한다. 즉, 컴포넌트 코드를 한줄 한줄 읽으면서 최신 가상 돔 객체(props, style 등이 포함)로 만든다. 만들때 useEffect의 코드가 실행된다.
...
렌더 Phase에서 얻은 Effect list를 토대로 Real DOM에 반영한다. 이때 ref를 심는다.
